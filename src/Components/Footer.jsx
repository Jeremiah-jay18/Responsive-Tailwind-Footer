...
const info=[
{id:'1',title:'Support', 
data:['Contact Us', 'FAQ', 'Download Oath Form','Download Registration Form']},
{id:'2',title:'Valdymas', data:['Programs', 'About', 'Blog','Project Read']}
]
...
const boxTwo= info.map((item,i)=>{
    return <div key={item.id} className='sm:mt-3 border sm:-mx-3'>
      <div className={`flex justify-between cursor-pointer border-left-black content-center sm:pt-2`} onClick={()=>toggleAccordion(i)}  >
         ...
         {selected===i?<IoIosArrowUp size={26} />:<IoIosArrowDown size={26} />}
             ...
             <div className={`px-6 ${selected===i?'h-36 transition-all duration-2000 ease-linear':' overflow-hidden h-0 transition-all duration-2000 ease-linear'}`}>
...
{/* General Blocks Wrapper */}
        <div className="foot-wrapper lg:flex lg:justify-between border sm:block ">
{/* Between Valdymas block and Info block */}
      <div className="sm:px-5 px-20 py-16 flex justify-between lg:gap-10 border sm:block sm:py-6 ">
 ...
 <div className=" form sm:text-center ">
    <form action=''>
        <div className="flex gap-4 mb-10 sm:justify-center">
            ...
            <h2 className='text-md font-bold'>Stay Up To Date On The Latest From Valdymas</h2>
        </div>
        <div className="flex gap-4 mb-4">
            <input type="text" className="placeholder-gray-500 bg-black border sm:w-full
        ...
        <input type="email" className="placeholder-gray-500 bg-black shadow-md border 
         ...
         <input type="submit" value="Sign Up" className='bg-red-700 py-3 cursor-pointer px-20 mt-3 ' />
         </div>
    </form>
 </div>
